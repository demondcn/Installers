(() => {
  ...
  const SUCCESS_SOUND = chrome.runtime.getURL('sounds/success.wav');
  ...
  function armVerification() {
    ...
    // Desbloquea WebAudio con la interacción del usuario y precarga el WAV.
    try {
      if (!audioCtx) {
  ...
  async function preloadSuccessSound() {
    ...
    if (!response.ok) throw new Error(`No se pudo cargar el WAV (${response.status})`);
    const bytes = await response.arrayBuffer();
    decodedSuccessBuffer = await audioCtx.decodeAudioData(bytes.slice(0));
    return decodedSuccessBuffer;
  }

  async function playPackagedWav() {
    if (!audioCtx) {
      audioCtx = new (window.AudioContext || window.webkitAudioContext)();
    }
    if (audioCtx.state === 'suspended') {
  ...
  async function playSuccessSound() {
    ...
    try {
      await playPackagedWav();
    } catch (err) {
      console.debug('[UdeC sonido] Falló el WAV, usando ding de respaldo:', err);
      try {
