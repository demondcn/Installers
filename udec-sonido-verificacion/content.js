(() => {
  ...
  function armVerification() {
    ...
    // El clic/Enter del usuario desbloquea WebAudio en navegadores Chromium.
    try {
      if (!audioCtx) {
  ...
  function pageLooksSuccessful() {
    ...
    // La ficha exitosa mostrada por la plataforma contiene estos tres campos.
    const hasProfileFields = SUCCESS_WORDS.every((word) => text.includes(word));
    if (!hasProfileFields) return false;

    // Evita disparar si únicamente existe el formulario inicial.
  ...
  async function playSuccessSound() {
    ...
    try {
      ...
      console.debug('[UdeC sonido] No fue posible reproducir el sonido:', err);
    }
  ...
  // Lectores de código de barras suelen terminar el código con ENTER.
  document.addEventListener(
    'keydown',
    (event) => {
  ...
  // Si la plataforma actualiza la ficha por AJAX, detecta el cambio inmediatamente.
  ...
  // Si el formulario hizo una navegación/recarga, recupera la marca guardada.
  if (isArmed()) {
    scheduleCheck(250);
